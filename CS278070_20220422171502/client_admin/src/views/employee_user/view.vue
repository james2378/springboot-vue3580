		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','age') || $check_field('add','age') || $check_field('set','age')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="年龄" prop="age">
					<el-input id="age" v-model="form_sub['age']" placeholder="请输入年龄"
							  v-if="user_group === '管理员' || (form_sub['employee_user_id'] && $check_field('set','age')) || (!form_sub['employee_user_id'] && $check_field('add','age'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','id') || $check_field('add','id') || $check_field('set','id')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="身份证" prop="id">
					<el-input id="id" v-model="form_sub['id']" placeholder="请输入身份证"
							  v-if="user_group === '管理员' || (form_sub['employee_user_id'] && $check_field('set','id')) || (!form_sub['employee_user_id'] && $check_field('add','id'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','department_name') || $check_field('add','department_name') || $check_field('set','department_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="部门名" prop="department_name">
					<el-select id="department_name" v-model="form_sub['department_name']"
						v-if="user_group === '管理员' || (form_sub['employee_user_id'] && $check_field('set','department_name')) || (!form_sub['employee_user_id'] && $check_field('add','department_name'))">
						<el-option v-for="o in list_department_name" :key="o['department_name']" :label="o['department_name']"
							:value="o['department_name']">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','department_name')">{{form_sub['department_name']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','position_name') || $check_field('add','position_name') || $check_field('set','position_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="职位名" prop="position_name">
					<el-select id="position_name" v-model="form_sub['position_name']"
						v-if="user_group === '管理员' || (form_sub['employee_user_id'] && $check_field('set','position_name')) || (!form_sub['employee_user_id'] && $check_field('add','position_name'))">
						<el-option v-for="o in list_position_name" :key="o['position_name']" :label="o['position_name']"
							:value="o['position_name']">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','position_name')">{{form_sub['position_name']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','entry_time') || $check_field('add','entry_time') || $check_field('set','entry_time')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="入职时间" prop="entry_time">
					<el-date-picker :disabled="disabledObj['entry_time_isDisabled']" v-if="user_group === '管理员' || (form_sub['employee_user_id'] && $check_field('set','entry_time')) || (!form_sub['employee_user_id'] && $check_field('add','entry_time'))" id="entry_time"
						v-model="form_sub['entry_time']" type="date" placeholder="选择日期">
					</el-date-picker>
					<div v-else-if="$check_field('get','entry_time')">{{form_sub['entry_time']}}</div>
				</el-form-item>
			</el-col>
...
<script>
	...
	export default {
        ...
        if (this.form["entry_time"].indexOf("-")===-1){
          this.form["entry_time"] = this.$toTime(parseInt(this.form["entry_time"]),"yyyy-MM-dd")
        }
				$.db.del("form");
				return param;
			},
        ...
        if(this.form["entry_time"]=="0000-00-00"){
          this.form["entry_time"] = null;
        }
				if(parseInt(this.form["entry_time"]) > 9999){
					this.form["entry_time"] = this.$toTime(parseInt(this.form["entry_time"]),"yyyy-MM-dd")
				}
